...
함수나 변수 이름조차 좀더 중립적으로 가져간다. 클래스 디자인할시에 앞으로의 상속을 염두해두어서 상속에 친화적인 코드로 만든다. 즉, 예를 들어서 isWahtIWnat에 굳이 if 문 안에 함수를 넣어서 확인하는 것. 사실 저거는 직사각형까지 고려한 코드가 된다. 상속시에 유리한 코딩을 미리밀 하는게 좋다.
